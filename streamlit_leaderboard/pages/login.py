...
st.error("Thank you so much||| Times up!!")
st.stop()
def authenticate_user(teamname:str, username:str, password:str) -> bool:
    ...
    if (creds == password):
        ...
        st.session_state["NameOfTeam"] = st.secrets.NamesOfTeam[f"{teamname}"]
        st.session_state["NameOfUser"] = st.secrets.NamesOfUsers[f"{teamname}_{username}"]
        ...
        if (config["status"] == "LOGGED_IN"):
            st.error("Seems like you have already logged in!!! Cannot open two sessions at the same time", icon = "❌")
            st.error("Close Previous Session below go to logout page in left", icon = "❌")
            st.stop()
        else:
            st.session_state["OPENAI_API_KEY"] = st.secrets.openAI_keys[f'{teamname}_{username}']
            with open(os.path.join(st.session_state.userDir, 'status.json'), 'w') as jwfile:
                config["status"] = "LOGGED_IN"
                json.dump(config, jwfile)
            st.success(f"Great you are logged in!! Welcome {st.session_state.NameOfUser} 🥷from {st.session_state.NameOfTeam} 👥")
            st.balloons()
    else:
...
if (st.session_state.get("username")):
    ...
    st.container()
    st.markdown("## List down all the submission information")
    st.stop()
